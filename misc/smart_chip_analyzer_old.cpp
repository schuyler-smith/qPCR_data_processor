...
int main(int argc, char *argv[])
{
    ...
    desc.add_options()
        ...
        ("input, i", po::value<std::string>(&input)->required(), "Input csv file. Output from the smartchip qPCR.")
        ("assays, a", po::value<std::string>(&assays)->required(), "Csv file mapping metadata for each assay.")
        ("output, o", po::value<std::string>(&output), "Output filepath and prefix to use. Default uses input file.")
        ("value, v", po::value<std::string>(&value)->default_value("Ct"), "Column name in input file to use for the Ct.")
        ("neg_control, n", po::value<std::string>(&neg_control)->default_value("NEG"), "Sample identifiers for the negative controls.")
        ("standard, s", po::value<std::string>(&standard)->default_value("STD"), "Sample identifiers for the standards.")
        ("non_template, c", po::value<std::string>(&non_template)->default_value("NTC"), "Sample identifiers for the non-template controls.")
        ("headers, j", po::value<bool>(&headers)->default_value(true), "Whether the input file has a header row. (bool)")
    ;
    ...
    // check help call
    if (vm.count("help")) {  
        std::cout << "\nUsage: smartchip_analyzer --input <smartchip_output.csv> --assays <assays.csv> --output <output_file_path>.\n" << std::endl;
        std::cout << desc << "\n";
        return 0;
    }
    // check arguments
    try {
        po::notify(vm);
        if (!vm.count("output")) {
            std::cout << "Use default output? [y/N]";
            std::cin >> output;
